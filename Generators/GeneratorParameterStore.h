...
// TODO: This really wants to become a single method with a flag to say where the value has to go.
template <typename T>
void GeneratorParameterStore::PushParameter(UnmaskedTestParameter Param, T Value) {
    if (SetContainsParameter(LegalIntegerParameters, Param)) {
        IntegerParameters[Param] = Optional<int>(Value, true);
    }
    else {
        Log(std::cout, WARN, "Parameter was pushed to GeneratorParameterStore that it wasn't expecting");
...
template <typename T>
void GeneratorParameterStore::PushTempParameter(UnmaskedTestParameter Param, T Value) {
    if (SetContainsParameter(LegalIntegerParameters, Param)) {
        TemporaryIntegerParameters[Param].emplace(int(Value));
    }
    else {
        Log(std::cout, WARN, "Temporary parameter was pushed to GeneratorParameterStore that it wasn't expecting");
        Log(std::cout, WARN, "Parameter was of type");
        Log(std::cout, VALUE_OUTPUT, std::to_string(Param));
    }
}
