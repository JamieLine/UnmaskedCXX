// Substitution values
// LinearCombinationWrong: The name of the tested CPP function.
// int: The return type of the function LinearCombinationWrong
// int,int,int,int: The types of each argument of LinearCombinationWrong in order.
...
bool SampleCXXProject_Calculations2() {
	...
	for (int i = 0; i < 10; i++) {
		...
		ReturnedValues[i] = LinearCombinationWrong(0,std::get<1>(GeneratedArguments[i]),0,std::get<3>(GeneratedArguments[i]));
	}
	...
	if (std::all_of(ReturnedValues, ReturnedValues+10, [ReturnedValues](int x){ return x == ReturnedValues[0];})) {
		...
		for (int i = 0; i < 10; i++) {
			...
			std::cout << 0;
 			std::cout << ",";
			std::cout << std::get<3>(GeneratedArguments[i]); 
 			std::cout << ",";
...
/*int main(int argc, char** argv) {
	SampleCXXProject_Calculations2();
}*/